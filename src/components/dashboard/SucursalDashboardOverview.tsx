...
import { KPI_TITLE_CLASS } from "@/lib/headings";
import {
  Bar,
  BarChart,
  CartesianGrid,
...
const FONT_UI = "var(--font-sans), ui-sans-serif, system-ui, sans-serif";
...
export function SucursalDashboardOverview({
  ...
  const { data: kpisData, isLoading: isLoadingKpis, error: kpisError } = useSWR(
    scopeKey ? ["dashboard-kpis", scopeKey] : null,
    async () => {
      const result = await getDashboardKPIs(userId ?? null, franquiciaId);
  ...
  const { data: vencimientosData, isLoading: isLoadingVencimientos, error: vencimientosError } =
    useSWR(
      scopeKey ? ["dashboard-vencimientos", scopeKey] : null,
      async () => {
        const result = await getDashboardProximosVencimientos(userId ?? null, 5, franquiciaId);
        if (!result.ok) throw new Error(result.error);
        return result.data;
      },
      swrSilentOptions,
  ...
  const { data: chartDataResponse, isLoading: isLoadingChart, error: chartError } = useSWR(
    scopeKey ? ["dashboard-chart", scopeKey] : null,
    async () => {
      const result = await getDashboardChartData(userId ?? null, franquiciaId);
      if (!result.ok) throw new Error(result.error);
      return result.data;
    },
    swrSilentOptions,
  ...
  const { data: movimientosData, isLoading: isLoadingMovimientos, error: movimientosError } = useSWR(
    scopeKey ? ["dashboard-movimientos", scopeKey] : null,
    async () => {
      const result = await getDashboardUltimosMovimientos(userId ?? null, 5, franquiciaId);
      if (!result.ok) throw new Error(result.error);
      return result.data;
    },
    swrSilentOptions,
  ...
  const chartFetchCompleted = chartDataResponse !== undefined;
    ...
    <div className="mt-8 space-y-6 md:space-y-8">
      ...
      <article className="rounded-2xl border border-zinc-800/50 bg-card p-5 md:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          <SectionHeading as="h3">Próximos egresos</SectionHeading>
            ...
            <table className="w-full text-sm">
              <thead className="bg-zinc-900">
                <tr className="border-b border-zinc-800/50">
                  <th className="px-3 py-2 text-left uppercase text-sm font-semibold tracking-wider text-zinc-400">
                    Concepto
                  </th>
                  <th className="px-3 py-2 text-left uppercase text-sm font-semibold tracking-wider text-zinc-400">
                    Descripción
                  </th>
                  <th className="px-3 py-2 text-right uppercase text-sm font-semibold tracking-wider text-zinc-400">
                    Total
                  </th>
                  <th className="px-3 py-2 text-right uppercase text-sm font-semibold tracking-wider text-zinc-400">
                    Vencimiento
                ...
                {isLoadingVencimientos && proximosVencimientos.length === 0 ? (
                  [...Array.from({ length: 3 })].map((_, idx) => (
                    <tr key={`sk-venc-${idx}`}>
                      ...
                      <td className="px-3 py-3 text-right">
                        <Skeleton className="ml-auto h-4 w-20 bg-zinc-800" />
                      </td>
                      <td className="px-3 py-3 text-right">
                        <Skeleton className="ml-auto h-4 w-20 bg-zinc-800" />
                      </td>
                  ...
                  proximosVencimientos.map((item) => (
                    <tr key={item.id} className="border-b border-zinc-800/40 last:border-b-0">
                      ...
                      <td className="px-3 py-2 text-right font-semibold tabular-nums text-[#e41b68]">
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:gap-8">
        <article className="min-w-0 rounded-2xl border border-zinc-800/50 bg-card p-5 md:p-6 lg:col-span-2">
          ...
          <p className="mt-1 text-sm text-foreground/70">Comparativa de los últimos 6 meses</p>
          <div className="mt-4 h-[300px] w-full min-w-0 sm:h-[320px]">
            {tieneDatosGrafico ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={[...chartData]} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <XAxis
                    dataKey="name"
                    tick={{ fill: "hsl(215 20% 72%)", fontSize: 12, fontFamily: FONT_UI }}
                    axisLine={{ stroke: "hsl(0 0% 100% / 12%)" }}
                    tickLine={false}
                  />
                  <YAxis
                    tick={{ fill: "hsl(215 20% 72%)", fontSize: 11, fontFamily: FONT_UI }}
                    axisLine={false}
                    tickLine={false}
                    tickFormatter={(v) =>
                  ...
                  <Tooltip
                    ...
                    itemStyle={{ fontFamily: FONT_UI, fontSize: 13 }}
                    formatter={(val, name) => {
            ...
            ) : isLoadingChart && !chartFetchCompleted ? (
              <div
        ...
        <article className="rounded-2xl border border-zinc-800/50 bg-card p-5 md:p-6 lg:col-span-1">
          ...
          <p className="mt-1 text-sm text-foreground/65">Entradas y salidas recientes</p>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 truncate text-foreground/85">{mov.detalle}</p>
                    </div>
                    <p
